...
def split_full_data():
    df_folds = pd.read_csv(f"{DATA}/external_upsampled_tabular.csv").rename(
        {"image_name": "image_id"}, axis=1
    )
    ...
    df_folds["anatom_site_general_challenge"] = df_folds[
        "anatom_site_general_challenge"
    ].fillna("unknown")
    df_folds["age_approx"] = df_folds["age_approx"].fillna(
        round(df_folds["age_approx"].mean())
    )
    patient_id_2_count = (
        df_folds[["patient_id", "image_id"]]
        .groupby("patient_id")
        .count()["image_id"]
        .to_dict()
    )
